...
class Notifier {
  constructor () {
    ...
    this.stackGap = 0;
    this.notificationHeight = 70;
    this.queue = [];
    this.startBarSizeEstimate = 70;
  ...
  notify (title, data) {
    ...
    let windowOptions = {
      width: 320,
      height: 70, /* stack gap now supported, though not sure why the current gap would not be sufficient */
      x: size.width - 320,
      y: size.height - 70,  /* this is because we set the poper position when we pop it from the queue */
      frame: false,
      transparent: false,
      resizable: false,
      alwaysOnTop: true,
  ...
  showNext () {
    ...
    for(var i = 0; i < this.activeNotifications.length ; i++) {
      ...
      notificationY += 12 + item.window.getBounds().height;
    }
      ...
      notificationWindow.on('close', () => {
        ...
        for(var i = 0; i < this.activeNotifications.length ; i++) {
          ...
          if(canMove) {
            ...
            var itemHeight = item.window.getBounds().height + 12;
